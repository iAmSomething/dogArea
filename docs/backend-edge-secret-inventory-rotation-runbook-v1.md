...
| `GEMINI_KEY` | `edge_runtime_secret` | `caricature` legacy alias | Edge env only | backend operator | legacy env compatibility only | sunset 대상 |
...
| `caricature` | `SUPABASE_URL` | `SUPABASE_ANON_KEY` | `SUPABASE_SERVICE_ROLE_KEY`, `OPENAI_API_KEY`/`GEMINI_API_KEY` | `GEMINI_KEY` | missing Supabase secret => `SERVER_MISCONFIGURED`, missing provider key => provider fallback or `ALL_PROVIDERS_FAILED` |
...
- `GEMINI_KEY`는 compatibility alias로만 유지합니다.
- 둘을 다른 값으로 동시에 운용하지 않습니다.
...
| `GEMINI_API_KEY` + `GEMINI_KEY` 모두 missing | Gemini path 완전 비활성 |
...
- `GEMINI_KEY`는 이전 배포/로컬 환경 호환용 alias
- 신규 문서/스크립트/secret store에는 `GEMINI_API_KEY`를 우선 사용
- compatibility alias 제거는 별도 후속 작업에서 진행

후속:

- `#479` backend remove legacy Gemini key alias after secret inventory cleanup
