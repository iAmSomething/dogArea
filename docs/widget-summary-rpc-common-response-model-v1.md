...
| --- | --- | --- | --- | --- |
| `rpc_get_widget_territory_summary` | canonical `payload jsonb` + positional `now_ts` compat | `today_tile_count`, `weekly_tile_count`, `defense_scheduled_tile_count`, `score_updated_at` | `has_data`, `refreshed_at` | app/widget member |
| `rpc_get_widget_hotspot_summary` | canonical `payload jsonb` + positional `radius_km`, `now_ts` compat | `signal_level`, `high_cells`, `medium_cells`, `low_cells`, `delay_minutes`, `privacy_mode`, `suppression_reason`, `guide_copy` | `has_data`, `is_cached`, `server_policy`, `refreshed_at` | app/widget anon/member |
| `rpc_get_widget_quest_rival_summary` | canonical `payload jsonb` + positional `in_now_ts` compat | `quest_*`, `rival_rank`, `rival_league` | `has_data`, `refreshed_at` | app/widget member |

## 현재 차이점

### 1. request contract는 canonical + compat 2계층입니다

- 세 RPC 모두 `payload jsonb` wrapper canonical 경로가 존재합니다.
- `territory` / `hotspot` / `quest_rival` 모두 positional compat 경로를 유지합니다.
...
- `territory` / `hotspot`에 `payload jsonb` wrapper canonical 추가 완료
- 세 RPC 모두 envelope response를 반환하는 canonical path 도입
- 기존 top-level shape는 compat path로 유지

### Phase 3. 앱 dual-read

- 앱 서비스가 아래 둘 다 읽도록 전환 완료
  - legacy top-level
...
- canonical envelope rollout 이후 smoke 2회 이상 안정화 확인
- implementation issue: `#459`
- positional compat sunset 시점 재평가
