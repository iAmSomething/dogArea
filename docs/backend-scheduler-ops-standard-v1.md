...
| `realtime.retention_cleanup` | writer | `rpc_cleanup_realtime_retention` + `view_realtime_retention_delete_debt` | 매시 `17분` | 다음 시간 1회 | 안전 재실행 |
...
### 5. `realtime.retention_cleanup`

근거:

- migration: `supabase/migrations/20260309043000_realtime_retention_cleanup_rollout.sql`
- RPC: `public.rpc_cleanup_realtime_retention(in_now_ts timestamptz default now())`
- verification view: `public.view_realtime_retention_delete_debt`
- retention baseline: `docs/backend-realtime-moderation-retention-policy-v1.md`

실행 기준:

- 매시 `17분 UTC`
- `walk_live_presence`처럼 초단위 TTL 대상은 아니므로 매분이 아니라 시간 단위 cleanup이면 충분

재시도:

- 별도 retry queue 없음
- 한 번 실패하면 다음 시간 스케줄이 자연 재시도 역할
- 2회 연속 실패 또는 `view_realtime_retention_delete_debt.overdue_rows` 지속 증가 시 수동 개입

재실행 성격:

- retention cutoff 이전 row delete이므로 안전 재실행
- 동일 시각 재실행은 중복 삭제를 일으키지 않음
...
- `view_realtime_retention_delete_debt`에 overdue row가 계속 누적
- `cron.job`에 `realtime_retention_cleanup_hourly`가 없거나 비활성
