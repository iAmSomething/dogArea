...
- control page 메트릭: `시간`, `포인트`, `연결` 3개만 유지
- 배너: 최근 액션 피드백 1개만 info page에 노출
- 큐 상태 줄: `큐 n건`, `ACK 상태`, `동기화 시각`

## 구현 기준
- `ContentsViewModel`이 액션 상태기계와 중복 억제를 담당
- `ContentView`는 control page의 CTA와 info page의 feedback/card 구성을 담당
