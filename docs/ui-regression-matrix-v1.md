...
| `FR-TABROOT-001` | `docs/non-map-tab-root-top-inset-contract-v1.md`, `docs/non-map-custom-header-safearea-contract-v1.md`, `#630`, `#678` | 홈/산책 기록/라이벌/설정 루트 헤더가 같은 non-map top inset과 custom header container 계약을 따르는지 | `FeatureRegressionUITests/testFeatureRegression_NonMapTabRootHeadersStayBelowStatusBar`, `FeatureRegressionUITests/testFeatureRegression_HomeHeaderStaysBelowStatusBarWithLongNames`, `FeatureRegressionUITests/testFeatureRegression_RivalHeaderStaysBelowStatusBarWithLongSubtitle`, `FeatureRegressionUITests/testFeatureRegression_WalkListStickySectionHeaderStaysBelowStatusBar` | 작은 화면과 접근성 글자 크기에서 비지도 탭 루트 헤더가 status bar 아래에 안정적으로 시작하고 첫 커스텀 헤더가 같은 시작 간격을 쓰는지 확인 |
...
| `FR-WALK-002C` | `docs/walklist-top-safearea-contract-v1.md`, `#622` | 산책 기록 sticky section header가 스크롤 중 상단 safe area 아래에 유지되는지 | `FeatureRegressionUITests/testFeatureRegression_WalkListStickySectionHeaderStaysBelowStatusBar` | `thisWeek` 섹션 헤더가 상단에 pin 되었을 때 status bar와 겹치지 않는지 확인 |
