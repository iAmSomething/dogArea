...
| `FR-TABROOT-001` | `docs/non-map-tab-root-top-inset-contract-v1.md`, `#630` | 홈/산책 기록/라이벌/설정 루트 헤더가 같은 non-map top inset 계약을 따르는지 | `FeatureRegressionUITests/testFeatureRegression_NonMapTabRootHeadersStayBelowStatusBar` | 작은 화면과 접근성 글자 크기에서 비지도 탭 루트 헤더가 status bar 아래에 안정적으로 시작하는지 확인 |
