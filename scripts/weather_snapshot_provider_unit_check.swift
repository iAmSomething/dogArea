...
assertTrue(homeLifecycle.contains("refreshIndoorMissions()"), "home lifecycle should refresh shared home weather state through indoor mission refresh")
assertTrue(homeIndoorMissionFlow.contains("refreshWeatherSnapshot(now: now)"), "indoor mission flow should refresh weather snapshot state with the current timestamp")
