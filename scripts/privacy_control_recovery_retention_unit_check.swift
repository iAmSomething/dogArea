...
assertTrue(mapViewModel.contains("privacyControlStateStore.persistSharingEnabled(self.locationSharingEnabled, for: currentPresenceUserId())"), "map view model should persist sharing preference through the shared privacy state store")
...
assertTrue(notificationView.contains("SettingsPrivacyCenterEntryCardView"), "settings screen should expose the privacy center entry card")
