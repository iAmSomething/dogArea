...
assertTrue(home.contains("NonMapRootHeaderContainer {\n                homeHeaderSection"), "Home should wrap the first custom header in NonMapRootHeaderContainer")
assertTrue(!home.contains("HomeRootLayoutMetrics"), "Home should not keep a per-screen root header top padding enum")

assertTrue(walkList.contains("NonMapRootHeaderContainer {\n                    WalkListDashboardHeaderView("), "WalkList should wrap the dashboard header in NonMapRootHeaderContainer")
...
assertTrue(rival.contains("NonMapRootHeaderContainer {\n                    rivalHeaderSection"), "Rival should wrap the first custom header in NonMapRootHeaderContainer")
assertTrue(!rival.contains("RivalRootLayoutMetrics"), "Rival should not keep a per-screen root header top padding enum")

assertTrue(settings.contains("NonMapRootHeaderContainer {\n                    settingsHeader("), "Settings should wrap the first custom header in NonMapRootHeaderContainer")
...
assertTrue(doc.contains("NonMapRootHeaderContainer"), "Contract doc should describe the reusable header container")
