...
assertTrue(mapView.contains("openRepresentativeSeasonTileDetail"), "MapView should wire the representative season tile detail opener into the season overview flow")
...
assertTrue(summarySheet.contains("map.season.sheet.openDetail"), "Season overview sheet should expose an accessibility fallback action for opening detail")
...
assertTrue(uiTest.contains("testFeatureRegression_MapSeasonOverviewSheetSurfacesMeaningAndLegend"), "Feature regression UI test should support the summary overview flow")
...
assertTrue(doc.contains("overview sheet"), "Issue doc should mention the overview sheet fallback detail opener")
