...
assertTrue(utility.contains("resolvedPresentationHost(from: host)"), "share bridge should resolve the active presentation host from UIKit hierarchy")
assertTrue(utility.contains("presenter.present(controller, animated: true)"), "share bridge should present directly from the resolved host view controller")
assertTrue(utility.contains("controller.popoverPresentationController?.sourceView = presenter.view"), "share bridge should configure popover anchor from the resolved presenter")
