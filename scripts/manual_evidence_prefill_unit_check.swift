...
assertTrue(printerScript.contains("widget|auth-smtp"), "env printer should support widget and auth-smtp")
assertTrue(prefillScript.contains("fill_prefixed_value_if_empty"), "prefill script should only fill empty prefixed lines")
assertTrue(prefillScript.contains("manual_evidence_prefill_sources.sh"), "prefill script should source shared widget prefill helpers")
assertTrue(prefillScript.contains("DOGAREA_AUTH_SMTP_PROJECT"), "prefill script should support auth smtp env values")
assertTrue(prefillDoc.contains("print_manual_evidence_prefill_env.sh"), "prefill doc should describe env printer")
assertTrue(prefillDoc.contains("connected physical iPhone"), "prefill doc should describe widget device auto-detect")
...
assertTrue(statusDoc.contains("next-prefill-env"), "status doc should describe env printer guidance")
assertTrue(statusDoc.contains("next-prefill-existing"), "status doc should describe prefill command")
assertTrue(readme.contains("docs/manual-evidence-prefill-v1.md"), "README should reference prefill doc")
...
setenv("DOGAREA_DISABLE_WIDGET_PREFILL_AUTODETECT", "1", 1)
setenv("DOGAREA_WIDGET_PREFILL_DEVICE_OS_STUB", "iPhone 14 / iOS 18.7.3", 1)
setenv("DOGAREA_WIDGET_PREFILL_APP_BUILD_STUB", "1.0 (14)", 1)
...
assertTrue(widgetAction.contains("- Device / OS: iPhone 14 / iOS 18.7.3"), "widget prefill should fill device metadata from stub/autodetect source")
assertTrue(widgetAction.contains("- App Build: 1.0 (14)"), "widget prefill should fill app build metadata from stub/autodetect source")
