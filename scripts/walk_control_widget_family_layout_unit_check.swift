...
require(widget.contains("WidgetSurfacePage(budget: layoutBudget) {\n            EmptyView()"), "WalkControlWidget small layout should drop the separate badge row to protect compact height budget.")
require(widget.contains("VStack(alignment: .leading, spacing: 4)"), "WalkControlWidget small layout should use an explicit compact body stack.")
...
require(doc.contains("Do not reserve a separate badge row in `systemSmall`"), "doc should explain why small omits a separate badge row.")
require(doc.contains("body spacing must stay at `4pt`"), "doc should pin the compact body spacing policy.")
