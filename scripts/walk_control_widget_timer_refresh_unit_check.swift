...
assertTrue(doc.contains("action overlay: next refresh immediately after `expiresAt`"), "doc should require action overlay refresh at expiry.")
...
assertTrue(widget.contains("nextRefreshDateForActionState("), "widget provider should derive timeline refresh from action-state expiry.")
assertTrue(widget.contains("snapshot.normalizedActionState"), "widget provider should read normalized action state when calculating refresh.")
assertTrue(widget.contains("actionState.expiresAt"), "widget provider should respect action-state expiry timestamps.")
