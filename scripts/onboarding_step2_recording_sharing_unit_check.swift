...
assertTrue(mapViewModel.contains("let sharingPreference = privacyControlStateStore.loadSharingEnabled(for: currentPresenceUserId())"), "map feature flags should load sharing state from the shared privacy store")
assertTrue(privacyStore.contains("seededValue = false"), "shared privacy store should preserve sharing OFF as the safe default")
