...
assertTrue(controlSurfaceView.contains("WatchActionBannerView"), "control surface should keep feedback inside the same primary surface")
assertTrue(controlSurfaceView.contains(".inline"), "control surface should request inline banner styling")
assertTrue(statusSummaryView.contains("compactPetContext"), "status summary should expose a compact idle pet context instead of a separate top card")
