...
assertTrue(utility.contains("@Binding var isPresented: Bool"), "share sheet wrapper should own presentation state via binding")
assertTrue(utility.contains("resolvedPresentationHost(from: host)"), "share sheet wrapper should resolve a presentation host from the current UIKit hierarchy")
assertTrue(utility.contains("presenter.present(controller, animated: true)"), "share sheet wrapper should present UIKit share sheet from the resolved presenter directly")
assertTrue(utility.contains("enum ActivitySharePresentationResult"), "share utility should define canonical presentation results")
...
assertTrue(walkDetail.contains("ActivityShareSheet(isPresented: $detailViewModel.showShareSheet, items: shareItems)"), "walk detail should use direct share presenter bridge")
assertTrue(walkDetail.contains("handleSharePresentationResult"), "walk detail should handle completed/cancelled/failed share events")
...
assertTrue(walkListDetail.contains("ActivityShareSheet(isPresented: $showShareSheet, items: shareItems)"), "walk list detail should use direct share presenter bridge")
assertTrue(walkListDetail.contains("walklist.detail.share.presenter.active"), "walk list detail should expose share presenter regression marker")
assertTrue(walkListDetail.contains("handleSharePresentationResult"), "walk list detail should distinguish share outcomes")
...
assertTrue(shareDoc.contains("빈 호스트"), "share doc should mention blank host regression prevention")
