...
assertTrue(contentView.contains("ScrollView"), "watch content view should keep a scroll container for the information surface")
assertTrue(contentView.contains("safeAreaInset(edge: .bottom"), "watch content view should keep the bottom dock safe area inset for the control surface")
assertTrue(contentView.contains("selectedSurface == .control"), "watch content view should show the bottom dock only on the control surface")
assertTrue(contentView.contains("WatchMainStatusSummaryView"), "watch content view should render a dedicated status summary view on the control surface")
...
assertTrue(doc.contains("safeAreaInset(edge: .bottom)"), "watch main overflow doc should still document the bottom dock policy")
