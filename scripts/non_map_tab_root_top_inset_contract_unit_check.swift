...
assertTrue(rival.contains(".appTabRootScrollLayout(extraBottomPadding: AppTabLayoutMetrics.comfortableScrollExtraBottomPadding)"), "Rival should use the shared non-map root top inset contract")
...
assertTrue(walkList.contains(".appTabRootScrollLayout(extraBottomPadding: AppTabLayoutMetrics.comfortableScrollExtraBottomPadding)"), "WalkList should use the shared non-map root top inset contract")
assertTrue(settings.contains(".appTabRootScrollLayout(extraBottomPadding: AppTabLayoutMetrics.comfortableScrollExtraBottomPadding)"), "Settings should use the shared non-map root top inset contract")

assertTrue(walkListHeader.contains("TitleTextView("), "WalkList header should use the shared title view")
assertTrue(walkListHeader.contains("accessibilityIdentifierPrefix: \"walklist.header\""), "WalkList header should expose stable accessibility identifiers")
assertTrue(walkListHeader.contains("-UITest.WalkListHeaderLongSubtitle"), "WalkList header should support the long subtitle regression route")
