...
assertTrue(scaffold.contains("nonMapRootTopSafeAreaPadding: CGFloat = 12"), "AppTabScaffold should define the compact non-map root top inset default")
...
assertTrue(home.contains(".appTabRootScrollLayout(") && home.contains("extraBottomPadding: AppTabLayoutMetrics.defaultScrollExtraBottomPadding") && home.contains("topSafeAreaPadding: 0"), "Home should move the root header into fixed top chrome")
...
assertTrue(rival.contains(".appTabRootScrollLayout(") && rival.contains("extraBottomPadding: AppTabLayoutMetrics.defaultScrollExtraBottomPadding") && rival.contains("topSafeAreaPadding: 0"), "Rival should use the shared compact bottom padding contract")
assertTrue(rival.contains(".nonMapRootTopChrome {"), "Rival should render the root header in fixed top chrome")
...
assertTrue(walkList.contains(".appTabRootScrollLayout(") && walkList.contains("extraBottomPadding: AppTabLayoutMetrics.defaultScrollExtraBottomPadding") && walkList.contains("topSafeAreaPadding: 0"), "WalkList should use the shared compact bottom padding contract")
assertTrue(walkList.contains(".nonMapRootPinnedHeaderLayout {"), "WalkList should render the root header in the pinned-header fixed chrome layout")
...
assertTrue(settings.contains(".appTabRootScrollLayout(") && settings.contains("extraBottomPadding: AppTabLayoutMetrics.defaultScrollExtraBottomPadding") && settings.contains("topSafeAreaPadding: 0"), "Settings should use the shared compact bottom padding contract")
